...
class SQLiteOutput:
    """Metadata-typed, row-permissive SQLite output for one Carto table."""
    ...
    def reset(self) -> None:
        ...
        with self.connection:
            self.connection.execute(f"DROP TABLE IF EXISTS {identifier(self.table)}")
            self.connection.execute(f"CREATE TABLE {identifier(self.table)} ({definitions})")
    ...
    def write(self, rows: Sequence[Mapping[str, Any]]) -> None:
        ...
        statement = (
            f"INSERT INTO {identifier(self.table)} ({column_sql}) VALUES ({placeholders})"
        )
...
class CSVOutput:
    ...
    def write(self, rows: Sequence[Mapping[str, Any]]) -> None:
        ...
        self.writer.writerows(
            {column: normalize(row.get(column)) for column in self.columns} for row in rows
        )
