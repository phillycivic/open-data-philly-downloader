...
class CartoClient:
    ...
    def query(self, sql: str) -> dict[str, Any]:
        try:
            response = self.session.post(self.base_url, data={"q": sql}, timeout=self.timeout)
    ...
    def iter_query(self, sql: str, *, page_size: int = 25_000) -> Iterator[list[dict[str, Any]]]:
        ...
        offset = 0
        while True:
            rows = self.query(f"{sql} LIMIT {page_size} OFFSET {offset}")["rows"]
            if not rows:
                return
            yield rows
