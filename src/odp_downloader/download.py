...
class Downloader:
    ...
    def fields(self, table: str, *, add_lat_lng: bool = True) -> dict[str, dict[str, Any]]:
        fields = dict(
            self.client.query(f"SELECT * FROM {identifier(table)} LIMIT 0")["fields"]
        )
    ...
    def download_by_datetime(
        ...
        *,
        ...
        if column not in fields:
            raise ValueError(f"Column {column!r} not found in {table!r}")
        expression = f"extract({granularity} from {identifier(column)})"
        return self._download_splits(
            table,
            fields,
            expression,
    ...
    def _download_splits(
        ...
        *,
        ...
        split_query = (
            f"SELECT DISTINCT {expression} AS split_value FROM {identifier(table)}"
    ...
    def _select_query(
        ...
        *,
        ...
        if order_by:
            ...
            for candidate in ("cartodb_id", "objectid"):
                if candidate in fields:
                    query += f" ORDER BY {identifier(candidate)}"
                    break
        return query
